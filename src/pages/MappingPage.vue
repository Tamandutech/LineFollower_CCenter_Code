<template>
  <div class="q-pa-md">
    <q-table title="Mapeamento" :rows="mappingRecords" :columns="columns" row-key="id" binary-state-sort>
      <template v-slot:top-left>
        <q-btn :icon="mdiSourceBranch" flat @click="toogleVersionsDialog(true)" :disable="!ble.connected" />
      </template>
      <template v-slot:body="props">
        <q-tr :props="props">
          ...
          <q-td key="Offset" :props="props">
            {{ props.row.offset }}
            <q-popup-edit v-model="props.row.offset" title="Atualizar Offset" buttons v-slot="scope">
              <q-input type="number" v-model="scope.value" dense autofocus />
            </q-popup-edit>
          </q-td>
    ...
    <div class="q-pa-md q-gutter-sm">
      <q-btn @click="
        ...
        " color="primary" label="Enviar mapeamento" :disable="loading !== null || mappingRecords.length === 0" />
      <q-btn @click="fetchMapping(false)" color="primary" label="Ler mapeamento" />
      <q-btn @click="fetchMapping(true)" color="primary" label="Ler mapeamento na Ram" />
      <q-btn @click="
        performAction(
        ...
        " color="primary" label="Salvar mapeamento" :disable="loading !== null" />
      <q-dialog v-model="showErrorDialog">
        <CommandErrorCard :error="error" />
      </q-dialog>
      <SuccessDialog v-model="showSuccessDialog" v-if="showSuccessDialog" :title="successDialogState.summary"
        :message="successDialogState.message" />
      <ConfirmActionDialog :confirm="confirm" v-if="isRevealed" :cancel="cancel" v-model="isRevealed">
        <template #title>{{ confirmDialogState.title }}</template>
        <template #question>{{ confirmDialogState.question }}</template>
      </ConfirmActionDialog>

      <ProfileVersionsDialog collection="mappings" title="Mapeamentos" :data="mappingRecords" @install-request="(version: Robot.ProfileVersion<Robot.Mapping>) =>
        performAction(
          withSuccessFeedback(
            sendMapping,
        ...
        " installable v-model="showVersionsDialog" v-slot="{ version }" v-if="showVersionsDialog">
        <q-list separator>
          <q-expansion-item switch-toggle-side v-for="(record, index) of version.data" :key="index"
    ...
    <div class="q-pa-md q-gutter-sm">
      <q-btn @click="
        ...
        " color="primary" label="Deletar Registro" />
      <q-btn @click="
        performAction(
        ...
        " color="primary" label="Deletar todos os registros" />
      <q-select v-model="deleteRecordId" :options="recordDeleteOptions" :disable="recordDeleteOptions.length == 0"
        label="Selecione o ID do registro que será deletado" />
    </div>
    <q-table title="Adicionar Registro" :rows="newRecord" :columns="newColumns" row-key="id" binary-state-sort>
      <template v-slot:body="props">
        <q-tr :props="props">
          <q-td key="EncMedia" :props="props">
            {{ props.row.encMedia }}
    ...
    <div class="q-pa-md q-gutter-sm">
      <q-btn @click="addMappingRecord" color="primary" label="Adicionar Registro" />
    </div>
  </div>
</template>
